"""Registered factorial validation of independent four-actor policies."""
import argparse
...
OUT=ROOT/"runs/autonomous-market-v15"
SPEC=ROOT/"experiment-specs/autonomous-market-v15/PREREGISTRATION.json"
...
def tasks():
    return [(r,s,p) for r in REGIMES for s in range(180201,180209) for p in POLICIES]+[("long",s,p) for s in range(180301,180305) for p in POLICIES]


def worker(task):
    regime,seed,policy=task;config=configuration(regime,policy);env=MarketEnv(config)
    state=env.reset(episode_id=f"four-actor-{regime}-{seed}",episode_seed=seed,max_rounds=60 if regime=="long" else 20,cooperation_mode="combined_v1")
    manifest=EpisodeManifest.create(env,state,experiment_id="autonomous-market-v15")
...
def prepare():
    ...
    spec={"tasks":tasks(),"policy_bits":["company_procurement","supplier_investment","government"],
        "source_hash":sha256_hash(source_files()),"config_hashes":{r+"-"+p:configuration(r,p).config_sha256 for r in (*REGIMES,"long") for p in POLICIES},
        ...
        "model_calls":0,"boundary":"Factorial diagnostic synthetic simulation. Report all welfare/exit differences including negative cells; no universal-profit or real-world calibration claim. Baseline is all three optional mechanisms off; supplier pricing and consumer choice remain active."}
