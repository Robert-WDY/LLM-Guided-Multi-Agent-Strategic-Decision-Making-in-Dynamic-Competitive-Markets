...
class ParetoAbstentionGate(StrictModel):
    """Reliability gate whose abstention never synthesizes an executable action."""

    gate_schema_version: Literal["pareto-reliability-gate-v3.0.0"] = (
        "pareto-reliability-gate-v3.0.0"
    )
    planner_decision_hash: str
    reliability_input_hash: str
    planner_candidate_id: str
    effective_candidate_id: str | None = None
    diagnostic_fallback_candidate_id: str
    withheld_candidate_id: str | None = None
    execution_disposition: Literal["recommend", "defer_to_agent"]
    fallback_is_proven_safe: Literal[False] = False
    safe_candidate_ids: list[str] = Field(min_length=1)
    excluded_candidates: list[ExcludedCandidate]
    opponent_model_confidence_ppm: int = Field(ge=0, le=PPM)
    top_two_value_gap_cents: int = Field(ge=0)
    ...
    @model_validator(mode="after")
    def validate_gate(self) -> "ParetoAbstentionGate":
        if self.should_abstain != bool(self.abstain_reason_codes):
            raise ValueError("reliability abstain reasons mismatch")
        if self.should_abstain:
            if (
                self.execution_disposition != "defer_to_agent"
                or self.effective_candidate_id is not None
                or self.withheld_candidate_id != self.planner_candidate_id
            ):
                raise ValueError("abstention must defer without an executable action")
        elif (
            self.execution_disposition != "recommend"
            or self.effective_candidate_id != self.planner_candidate_id
            or self.withheld_candidate_id is not None
        ):
            raise ValueError("released advice must bind the planner candidate")
        if (
            self.effective_candidate_id is not None
            and self.effective_candidate_id not in self.safe_candidate_ids
        ):
            raise ValueError("released reliability candidate must be gate-safe")
        excluded_ids = {item.candidate_id for item in self.excluded_candidates}
        if excluded_ids.intersection(self.safe_candidate_ids):
            raise ValueError("safe and excluded reliability candidates must be disjoint")
        if self.gate_hash != compute_abstention_gate_hash(self):
            raise ValueError("reliability abstention gate hash mismatch")
        return self
...
def compute_abstention_gate_hash(
    gate: ParetoAbstentionGate | Mapping[str, Any],
) -> str:
    payload = (
        gate.model_dump(mode="json")
        if isinstance(gate, ParetoAbstentionGate)
        else dict(gate)
    )
    payload.pop("gate_hash", None)
    return sha256_hash(
        {
            "hash_protocol_version": "pareto-reliability-gate-hash-v2.0.0",
            "gate": payload,
        }
    )
...
def build_abstention_gate(
    *,
    ...
    """Build v7's fail-closed gate without declaring a fallback safe by fiat."""

    evaluations = {
        item.candidate.candidate_id: item for item in plan.evaluations
    }
    if diagnostic_fallback_candidate_id not in evaluations:
        raise ValueError("reliability diagnostic fallback candidate is missing")
    planner_id = decision.recommended_candidate_id
    planner = evaluations[planner_id]
    fallback = evaluations[diagnostic_fallback_candidate_id]
    safe_ids = sorted(
        item.candidate_id
        for item in decision.candidate_assessments
        if item.eligible
    ...
    if not safe_ids:
        raise ValueError("reliability gate has no eligible candidate")
    ...
    should_abstain = bool(reasons)
    effective_id = None if should_abstain else planner_id
    gap_confidence = min(PPM, top_gap * PPM // max(1, uncertainty))
    advisor_confidence = min(opponent_confidence, gap_confidence)
    reliability_input_hash = sha256_hash(
        {
            "protocol": "pareto-reliability-public-input-v3.0.0",
            "public_decision_input_hash": public_decision_input_hash,
            "planner_decision_hash": decision.decision_hash,
    ...
    payload: dict[str, Any] = {
        "gate_schema_version": "pareto-reliability-gate-v3.0.0",
        "planner_decision_hash": decision.decision_hash,
        "reliability_input_hash": reliability_input_hash,
        "planner_candidate_id": planner_id,
        "effective_candidate_id": effective_id,
        "diagnostic_fallback_candidate_id": diagnostic_fallback_candidate_id,
        "withheld_candidate_id": planner_id if should_abstain else None,
        "execution_disposition": (
            "defer_to_agent" if should_abstain else "recommend"
        ),
        "fallback_is_proven_safe": False,
        "safe_candidate_ids": safe_ids,
        "excluded_candidates": [item.model_dump(mode="json") for item in excluded],
        "opponent_model_confidence_ppm": opponent_confidence,
        "top_two_value_gap_cents": max(0, top_gap),
