...
DEST = ROOT / "releases/local-market-v15.0.0"
...
def main():
    ...
    tests = OUTER / "migration_2026-09-07/theory-v15-final.xml"
    suite = ElementTree.parse(tests).getroot().find("testsuite")
    assert int(suite.get("tests")) == 452
    assert all(int(suite.get(k, "0")) == 0 for k in ("errors", "failures", "skipped"))
    qa = json.loads((ROOT / "runs/theory-v15-acceptance/acceptance.json").read_text(encoding="utf-8"))
    ...
    old = ROOT / "releases/local-market-v14.0.0/local-market-v14.0.0-source.zip"
    assert digest(old) == "fa4c93f6014951bac9b317650e67125acae19f7f65da670291bdcc470ff02555"
    ...
    archive = DEST / "local-market-v15.0.0-source.zip"
    ...
    for name in ("acceptance.json", "backup-restore.json"):
        shutil.copy2(ROOT / "runs/theory-v15-acceptance" / name, DEST / name)
    ...
    manifest = dict(release="local-market-v15.0.0", scope="single user local four-actor synthetic market with eight game-theory topics and five company policies", config="configs/market_v14_local.yaml", python=sys.version, source_files=hashes, outer_files={n: digest(OUTER / n) for n in outer}, archive_sha256=digest(archive), source_sha256=hashlib.sha256(json.dumps(hashes,sort_keys=True,separators=(",", ":")).encode()).hexdigest(), backend_tests=452, frontend_tests=11, browser_qa=True, repeated_episodes=576, repeated_rounds=34560, learning_runs=64, learning_rounds=192000, analytic_conditions=81, market_episodes=48, market_rounds=960, new_real_attempts=6, real_valid_choices=6, budget_reserved_cny=9.993565, excluded=["credentials", "runtime databases", "local research log", "venv", "node_modules", "generated frontend; rebuild with build_local.ps1"])
