"""Freeze accepted v17 source/evidence and verify every archived byte."""
import hashlib,json,shutil,sys,zipfile
from pathlib import Path
from xml.etree import ElementTree
from game_theory_agent.local_budget import status
ROOT=Path(__file__).resolve().parents[1];OUTER=ROOT.parent.parent;DEST=ROOT/'releases/local-market-v17.0.0';RUNS=ROOT/'runs/advisor-v17'
OLD={
 'local-market-v16.0.0':'9cd3c42c4c3755d02c4d688e6b008171fb0f657c4ea9cb903fe9ee94d137ba99',
...
def main():
 ...
 tests=OUTER/'migration_2026-09-07/advisor-v17-final.xml';suite=ElementTree.parse(tests).getroot().find('testsuite');assert int(suite.get('tests'))==471 and all(int(suite.get(k,'0'))==0 for k in ('errors','failures','skipped'))
 qa=read(RUNS/'acceptance.json');assert qa['passed'] and qa['live_api'] and qa['backup_restore'];assert qa['frontend']==dict(build=True,typecheck=True,lint=True,tests=16)
 stages=[read(RUNS/f'stage{i}.json') for i in range(1,7)];assert all(s['passed'] for s in stages)
 analysis=read(RUNS/'analysis.json');assert analysis['passed'] and analysis['variants']['v17']['cases']==96
 ...
 content={p.relative_to(ROOT).as_posix():p for p in files};assert all(k in content for k in ('src/game_theory_agent/game_theory/advisor_search.py','frontend/app/advisor-v16.tsx','docs/advisor-v17-results.md'))
 ...
 hashes={n:digest(p) for n,p in content.items()};DEST.mkdir(parents=True);archive=DEST/'local-market-v17.0.0-source.zip'
 ...
 with zipfile.ZipFile(archive) as bundle:
  ...
  for name in outer:assert hashlib.sha256(bundle.read(name)).hexdigest()==digest(OUTER/name)
 ...
 with zipfile.ZipFile(DEST/'advisor-v17-evidence.zip','x',zipfile.ZIP_DEFLATED) as bundle:
  for name,path in evidence.items():bundle.write(path,name)
 with zipfile.ZipFile(DEST/'advisor-v17-evidence.zip') as bundle:
  assert bundle.testzip() is None
  for name,path in evidence.items():assert hashlib.sha256(bundle.read(name)).hexdigest()==digest(path)
 for name in ('acceptance.json','backup-restore.json','analysis.json',*(f'stage{i}.json' for i in range(1,7))):shutil.copy2(RUNS/name,DEST/name)
 shutil.copy2(tests,DEST/'backend-tests.xml')
 manifest=dict(release='local-market-v17.0.0',scope='single-user local synthetic market; public price learning and finite response-aware advice; empirically evaluated, not globally optimal',config='configs/market_v14_local.yaml',python=sys.version,source_files=hashes,outer_files={n:digest(OUTER/n) for n in outer},archive_sha256=digest(archive),evidence_sha256=digest(DEST/'advisor-v17-evidence.zip'),evidence_files={n:digest(p) for n,p in evidence.items()},backend_tests=471,frontend_tests=16,stages=[s.get('cases') for s in stages],holdout_variant_cases=232,independent_holdout_seeds=32,holdout_v17=analysis['variants']['v17'],closed_loop_episodes=24,closed_loop_rounds=300,advisor_decisions=540,arithmetic_checks=analysis['arithmetic_checks'],new_real_calls=0,budget_reserved_cny=budget['reserved_cny'],global_optimality_proven=False,old_release_hashes=OLD,excluded=['credentials','runtime databases','local research log','venv','node_modules','generated frontend; rebuild with build_local.ps1'])
