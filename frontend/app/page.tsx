...
const PERSONA_WEIGHT_LABELS: Record<string, string> = {
  profit: "利润",
  growth: "增长",
  risk: "风险控制",
  cash: "现金安全",
...
const PERSONA_TRAIT_LABELS: Record<string, string> = {
  timeDiscount: "长期重视程度",
  riskAversion: "风险厌恶",
  cooperation: "合作倾向",
  opportunism: "机会主义倾向",
...
type BackendPersonaCatalog = {
  ...
  profiles: Array<{
    ...
    experiment_selectable: boolean;
  }>;
  demo_only_personas: Array<{
    persona_id: string;
    validation_status: string;
    experiment_selectable: boolean;
  }>;
...
function formatMoney(cents: number, compact = false) {
  if (compact)
    return `¥${new Intl.NumberFormat("zh-CN", { notation: "compact", maximumFractionDigits: 1 }).format(cents / 100)}`;
...
function AppShell({
  ...
  if (active === "setup")
    return (
      <main className="configuration-shell">
        <header className="simple-workspace-header">
          <button type="button" onClick={onHome}>
            ← 返回主页
          </button>
          <div>
            <span>{ENTRY_META[entryMode].eyebrow} · 环境配置</span>
            <strong>{ENTRY_META[entryMode].label}</strong>
          </div>
          <b>博弈实验室</b>
        </header>
        {children}
  ...
  const allowed =
    entryMode === "participant"
      ? ["live", "replay", "report"]
      : entryMode === "observer"
        ? ["live", "observatory", "communication", "market", "replay", "report"]
        : [
            "archive",
            "live",
            "observatory",
  ...
  const visibleNavigation = NAV_ITEMS.filter(
    (item) => allowed.includes(item.id) && (item.id !== "report" || completed),
  );
  return (
    <main className="lab-shell">
      <aside className="lab-sidebar">
        <button className="lab-brand" type="button" onClick={onHome}>
          <span className="brand-glyph">博弈</span>
          <div>
            <strong>多智能体博弈实验室</strong>
            <small>返回主入口</small>
          </div>
        </button>
        <div className="workspace-identity">
          <span>{ENTRY_META[entryMode].eyebrow}</span>
          <strong>{ENTRY_META[entryMode].label}</strong>
          <button type="button" onClick={() => setActive("setup")}>
            修改环境
          </button>
        </div>
        <nav aria-label="当前模式导航">
        ...
        <div className="sidebar-run-card">
          <span>当前会话</span>
          <strong>{episodeId || "尚未创建"}</strong>
          <div>
            <StatusDot
              tone={
                runtimeMode === "backend"
        ...
        <header className="lab-topbar">
          <div>
            <span className="topbar-kicker">
              {ENTRY_META[entryMode].eyebrow} · {nav?.hint}
            </span>
            <h1>{nav?.label}</h1>
          </div>
          <div className="topbar-meta">
            <span>
              <StatusDot tone={backendOnline ? "ok" : "warn"} />
              {backendOnline ? "后端已连接" : "当前为演示模式"}
            </span>
            <button type="button" onClick={onHome}>
              返回主页
            </button>
  ...
  return (
    <div className="landing-page">
      ...
      <section className="landing-hero">
        ...
        <p>
          先选择你要扮演的角色，再配置市场环境。观察、参与和研究不会混在同一个界面里。
        </p>
      </section>
      <section className="entry-grid">
        <button
          type="button"
          className="entry-card participant"
          onClick={() => choose("participant")}
        >
          <i>01</i>
          <span>参与决策</span>
          <h2>个人体验</h2>
          <p>作为公司 A 做价格、投入与合作决策，观察其他智能体如何响应。</p>
          <b>
            配置并进入 <em>→</em>
          </b>
        </button>
        <button
          type="button"
          className="entry-card observer"
          onClick={() => choose("observer")}
        >
          <i>02</i>
          <span>观察过程</span>
          <h2>观察实验</h2>
          <p>不参与决策，跟随回合查看市场、消息、智能体判断和结算结果。</p>
          <b>
            配置并进入 <em>→</em>
          </b>
        </button>
        <button
          type="button"
          className="entry-card research"
          onClick={() => choose("research")}
        >
          <i>03</i>
          <span>研究分析</span>
          <h2>研究控制台</h2>
          <p>配置处理组、回合重建和完整研究工具；实验完成后才生成报告。</p>
          <b>
            配置并进入 <em>→</em>
          </b>
        </button>
      ...
      <footer className="landing-footer">
        <span>后端市场环境是权威结果来源</span>
        <span>不展示模型隐藏思维过程</span>
        <span>交互演示不等于研究证据</span>
      </footer>
    </div>
  );
}
...
function SetupView({
  ...
  personaCatalogVerified: boolean;
}) {
  const editor =
    agents.find((agent) => agent.companyId === editingAgent) ?? null;
  const patchAgent = (companyId: string, patch: Partial<AgentConfig>) =>
    setAgents((current) =>
      current.map((agent) =>
        agent.companyId === companyId ? { ...agent, ...patch } : agent,
      ),
    );
  function removeAgent(companyId: string) {
    setAgents((current) =>
      current.filter((agent) => agent.companyId !== companyId),
    );
    if (editingAgent === companyId) setEditingAgent(null);
  }
  function addAgent() {
    if (agents.length < MAX_COMPANIES)
      setAgents((current) => [
        ...current,
  ...
  return (
    <div className="view-pad setup-view">
      <section className="setup-hero">
        <div>
          <span>{ENTRY_META[entryMode].eyebrow} · 环境</span>
          <h2>配置环境</h2>
          <p>
            当前入口：<b>{ENTRY_META[entryMode].label}</b>
            。只需确认市场与智能体；进入后会打开独立界面，随时可以返回主页。
          </p>
        </div>
      ...
      <div className="setup-grid">
        <section className="card market-config-card">
          <SectionHead
            eyebrow="01 · 市场"
            title="市场配置"
            description="进行对照实验时，保持随机种子和市场不变，只改变需要研究的处理条件。"
          />
          <div className="segmented-label">信息可见范围</div>
          <div className="segmented three">
            {(["perfect", "public", "imperfect"] as const).map((mode) => (
              <button
                key={mode}
                type="button"
                className={config.informationMode === mode ? "active" : ""}
                onClick={() =>
                  setConfig((current) => ({
                    ...current,
          ...
          <label className="field">
            ...
            <select
              ...
              onChange={(event) =>
                setConfig((current) => ({
                  ...current,
                  marketType: event.target.value as LabConfig["marketType"],
                }))
              }
            >
            ...
            <label className="field">
              ...
              <select
                ...
                onChange={(event) =>
                  setConfig((current) => ({
                    ...current,
                    rounds: Number(event.target.value) as LabConfig["rounds"],
                  }))
                }
              >
          ...
          <div className="capability-list">
            ...
            <div>
              <span>
                <b>智能体通信</b>
                <small>公开消息与点对点私信</small>
              </span>
              <Toggle
                checked={config.communication}
                label="启用通信"
                onChange={(value) =>
                  setConfig((current) => ({ ...current, communication: value }))
                }
              />
            </div>
            <div>
            ...
            <div>
              ...
              <Toggle
                ...
                label="启用博弈论增强"
                onChange={(value) =>
                  setConfig((current) => ({ ...current, gameTheory: value }))
                }
              />
            </div>
        ...
        <section className="card agent-config-card">
          <SectionHead
            ...
            action={
              <button
                ...
                type="button"
                disabled={agents.length >= MAX_COMPANIES}
                onClick={addAgent}
              >
                ＋ 添加智能体
              </button>
          ...
          <div className="agent-config-list">
            {agents.map((agent) => (
              ...
              >
                <span className="agent-letter">{agent.shortName}</span>
                <div className="agent-identity">
                  <strong>{agent.companyName}</strong>
                  <small>{agent.companyId}</small>
                </div>
                <label>
                  <span>控制方式</span>
                  <select
                    value={agent.driver}
                    onChange={(event) => {
                      const driver = event.target
                        .value as AgentConfig["driver"];
                      patchAgent(agent.companyId, {
                        driver,
                        model: DRIVER_MODELS[driver],
                      });
                    }}
                  >
                    <option value="human">人类参与者</option>
                    <option value="doubao">豆包模型</option>
                    <option value="deepseek">深度求索模型</option>
                ...
                <label>
                  ...
                  <button
                    ...
                    onClick={() => setEditingAgent(agent.companyId)}
                  >
                    {personaLabels[agent.persona] ??
                      PERSONAS[agent.persona].label}
                    <i>↗</i>
                  </button>
                </label>
                <div className="agent-flags">
                  <span>
                    {agent.information === "full"
                      ? "完整信息"
                      : agent.information === "public"
                        ? "公共信息"
                        : "不完全信息"}
                  </span>
                  <span className={agent.communication ? "on" : ""}>通信</span>
                  <span className={agent.gameTheory ? "on" : ""}>博弈辅助</span>
                </div>
                {agents.length > 2 && (
                  <button
                    className="remove-agent"
                    type="button"
                    aria-label={`移除 ${agent.companyName}`}
                    onClick={() => removeAgent(agent.companyId)}
                  >
                    ×
                  </button>
                )}
      ...
      <section className="launch-bar">
        <div className="controller-field">
          ...
          <input
            type="password"
            placeholder="只保存在当前浏览器内存；高级后端实验必填"
            value={config.controllerToken}
            onChange={(event) =>
              setConfig((current) => ({
                ...current,
      ...
      {editor && (
        <div
          ...
          role="presentation"
          onMouseDown={() => setEditingAgent(null)}
        >
          <aside
          ...
          >
              ...
              >
                {personaOptions.map((personaId) => (
                  <option key={personaId} value={personaId}>
                    {personaLabels[personaId] ?? PERSONAS[personaId].label}
                  </option>
                ))}
              </select>
            </label>
            <p className="persona-summary">
              {PERSONAS[editor.persona].summary}
            </p>
            <div className="persona-section">
              <span>目标权重</span>
              {Object.entries(PERSONAS[editor.persona].weights).map(
                ([key, value]) => (
                  <div className="persona-slider" key={key}>
                    <label>
                      <b>{PERSONA_WEIGHT_LABELS[key]}</b>
                      <span>{value}%</span>
                    </label>
                    <input
                      type="range"
            ...
            <div className="persona-section">
              <span>行为倾向</span>
              {Object.entries(PERSONAS[editor.persona].traits).map(
                ([key, value]) => (
                  <div className="persona-slider" key={key}>
                    <label>
                      <b>{PERSONA_TRAIT_LABELS[key]}</b>
                      <span>{value}%</span>
                    </label>
                    <input
                      type="range"
...
function LiveView({
  ...
  const processAgent =
    agents.find((agent) => agent.companyId === processCompanyId) ?? agents[0];
  ...
  const visibleMessages = runtimeMode === "backend" ? [] : [
    ...
    {
      route: `${round % 3 === 0 ? "C" : "D"} → A`,
      text:
        round % 3 === 0
          ? "建议下一轮共同提高韧性贡献。"
          : "供应冲击可能高于公开预测。",
      trust: "PRIVATE · UNVERIFIED",
    },
  ];
  const processMessages =
    processAgent.companyId === human.companyId
      ? visibleMessages
      : [
          {
            route: `${processAgent.companyId.slice(-1)} → PUBLIC`,
            text: processAgent.decision.summary,
            trust: "UNVERIFIED",
          },
          {
            route: `A → ${processAgent.companyId.slice(-1)}`,
            text: "请说明下一轮是否会改变公开价格。",
            trust: "PRIVATE · UNVERIFIED",
          },
        ];
  const hasSettledRound = agents.some(
    (agent) => agent.shareDelta !== 0 || agent.profit !== 0,
  );
  const hasHistoricalEvidence = round > 1 || completed;
  const moveToNextRound = () => {
    nextRound(humanAction);
    setProcessStep(0);
  };
  return (
    <div className="view-pad live-view">
      <section className="live-command card">
        <div className="round-orbit">
          ...
          <i
            style={
              {
                "--progress": `${maxRounds === 1 ? 100 : Math.min(100, ((round - 1) / (maxRounds - 1)) * 100)}%`,
        ...
        <div className="phase-track">
          {phase.map((item, index) => (
            <div
              ...
              key={item}
            >
              <i>{completed || index < processStep ? "✓" : index + 1}</i>
              <span>{item}</span>
            </div>
          ))}
        </div>
        <div className="live-actions">
          <span>
            {runtimeMode === "backend" ? "后端真实回合" : "前端交互演示"}
            <small>{notice}</small>
          </span>
          <b className={completed ? "round-complete" : "round-open"}>
            {completed ? "本次体验已完成" : `第 ${round} 回合进行中`}
          </b>
        </div>
      </section>
      <div className="live-scope-banner">
        <div>
          <StatusDot tone={runtimeMode === "backend" ? "ok" : "warn"} />
          <span>
            <b>{runtimeMode === "backend" ? "权威后端状态" : "交互演示状态"}</b>{" "}
            · 消息是未验证陈述；对手私有信息不会进入你的事实状态。
          </span>
        </div>
      ...
      <div className="metrics-row">
        ...
        <MetricCard
          label="行业抗冲击能力"
          value={publicState[3]?.value ?? "—"}
          note="越高表示灾害或供应冲击造成的损失越小"
          accent
        />
        <MetricCard
          label="份额校验"
          value={`${shareTotal.toFixed(1)}%`}
          note={`${agents.length} 家公司市场份额必须合计为 100%`}
        />
      </div>
      <section className="agent-board clean">
        {agents.map((agent, index) => (
          <article
            className={`runtime-agent-card${interactive && index === 0 ? " human" : ""}`}
            key={agent.companyId}
            style={{ "--agent": agent.color } as React.CSSProperties}
          >
            <div className="runtime-agent-head">
              <span>{agent.companyId.slice(-1)}</span>
              <div>
                <strong>{agent.companyName}</strong>
                <small>{agent.persona}</small>
              </div>
              <b>{interactive && index === 0 ? "你" : "智能体"}</b>
            </div>
            <div className="agent-primary-result">
              <span>
                市场份额{" "}
                <i title="该公司获得的全部市场订单比例。全部公司合计为 100%。">
                  ?
                </i>
              </span>
            ...
            <div className="agent-secondary-result">
              <span>
                公开价格 <b>{formatMoney(agent.price)}</b>
              </span>
              <span title="数值越高，供应中断或灾害发生时受到的损失越小。">
                抗冲击能力 <b>{agent.resilience.toFixed(1)}%</b>
              </span>
            </div>
          </article>
        ))}
      </section>
      <section className="live-workbench-head">
        <div>
          <span>
            第 {round} 回合 · {interactive ? "个人决策" : "观察过程"}
          </span>
          <h2>{interactive ? "决策过程" : "智能体处理过程"}</h2>
          <p>
            选择公司，再按照六个步骤查看它看到了什么、如何交流、依据什么形成判断并做出动作。
          </p>
        </div>
        <div className="privacy-pill">只展示该公司合法可见的上下文</div>
      </section>
      <section className="card decision-process">
        <div className="process-toolbar">
          <div className="process-agent-tabs">
            {agents.map((agent) => (
              <button
                type="button"
                className={
                  processAgent.companyId === agent.companyId ? "active" : ""
                }
                key={agent.companyId}
                onClick={() => {
                  setProcessCompanyId(agent.companyId);
                  setProcessStep(0);
                }}
              >
                <i style={{ background: agent.color }}>
                  {agent.companyId.slice(-1)}
                </i>
                {agent.companyName}
              </button>
            ))}
          </div>
          <span>当前查看：{processAgent.companyName}</span>
        </div>
        ...
        <div className="process-context">
          {processStep === 0 && (
            <div>
              ...
              <h3>接收本轮可见信息</h3>
              <p>
                公司只能看到公共市场状态和自己的私有经营状态。对手现金、利润、成本和真实人格仍然隐藏。
              </p>
              <div className="context-facts">
                {processAgent.observation.public.slice(0, 3).map((item) => (
                  <article key={item.label}>
                    <span>{item.label}</span>
                    <b>{item.value}</b>
                  </article>
                ))}
                {processAgent.observation.private.slice(0, 2).map((item) => (
                  <article className="private" key={item.label}>
                    <span>{item.label} · 仅自己</span>
                    <b>{item.value}</b>
                  </article>
                ))}
              </div>
          ...
          {processStep === 1 && (
            <div>
              <span>这一步在做什么</span>
              <h3>读取公开消息和发给自己的私信</h3>
              <p>
                消息可能是合作建议、威胁或虚假声明，只能作为信号，不能覆盖事实状态。
              </p>
              <div className="process-message-flow">
                {processMessages.map((message, index) => (
                  <article key={message.route}>
                    <i>{index + 1}</i>
                    <div>
                      <b>{message.route.replace("PUBLIC", "所有公司")}</b>
                      <p>{message.text}</p>
                      <small>
                        {message.trust
                          .replace("UNVERIFIED", "内容未经验证")
                          .replace("PRIVATE", "私信")}
                      </small>
                    </div>
                  </article>
          ...
          {processStep === 2 && (
            <div>
              <span>这一步在做什么</span>
              <h3>根据已经发生的公开行为更新对手判断</h3>
              {!hasHistoricalEvidence ? (
                <div className="no-evidence">
                  <b>第一回合没有历史证据</b>
                  <p>
                    当前只能保持“未知”，不能显示精确的降价概率。第一轮结算后，系统才会根据公开价格和份额变化形成判断。
                  </p>
                </div>
              ) : (
                <div className="process-beliefs">
                  {processAgent.beliefs.map((belief) => (
                    <article key={belief.companyId}>
                      <b>{belief.companyId.slice(-1)}</b>
                      <div>
                        <span>下一轮降价可能性</span>
                        <strong>{belief.nextAction.cut}%</strong>
                        <p>{belief.evidence.join("；")}</p>
                      </div>
                    </article>
                  ))}
                </div>
          ...
          {processStep === 3 && (
            <div>
              <span>这一步在做什么</span>
              <h3>把目标、风险和对手判断整理成候选策略</h3>
              <p>
                策略建议不是命令，智能体可以采纳，也可以因为现金、风险或人格偏好而拒绝。
              </p>
              <div className="strategy-explanation">
                <article>
                  <span>当前目标</span>
                  <b>{processAgent.plan.goal}</b>
                </article>
                <article>
                  <span>系统建议价格</span>
                  <b>{formatMoney(processAgent.advisor.recommendedPrice)}</b>
                </article>
                <article>
                  <span>最重要的风险</span>
                  <b>{processAgent.decision.factors[2]}</b>
                </article>
              </div>
            </div>
          )}
          {processStep === 4 && (
            <div>
              <span>这一步在做什么</span>
              <h3>综合上下文后提交本轮动作</h3>
              <p>{processAgent.decision.summary}</p>
              <div className="final-action-context">
                <span>
                  价格 <b>{formatMoney(processAgent.action.price)}</b>
                </span>
                <span>
                  广告{" "}
                  <b>{formatMoney(processAgent.action.advertising, true)}</b>
                </span>
                <span>
                  服务投入{" "}
                  <b>{formatMoney(processAgent.action.service, true)}</b>
                </span>
                <span>
                  共享抗冲击投入{" "}
                  <b>{formatMoney(processAgent.action.contribution, true)}</b>
                </span>
              </div>
            </div>
          )}
          {processStep === 5 && (
            <div>
              <span>这一步在做什么</span>
              <h3>{agents.length} 家公司动作锁定后统一结算</h3>
              <p>
                市场份额是相对竞争结果：一家公司的份额增加，必然由其他公司份额减少来平衡。
              </p>
              <div className="settlement-explanation">
                <b>{processAgent.companyName}</b>
                <span>
                  份额变化 {processAgent.shareDelta >= 0 ? "+" : ""}
                  {processAgent.shareDelta.toFixed(1)} 个百分点
                </span>
                <span>本轮价格 {formatMoney(processAgent.action.price)}</span>
              </div>
            </div>
          )}
        </div>
        <div className="process-next">
          <span>
            步骤 {processStep + 1} / {phase.length}
          </span>
          <button
            type="button"
            disabled={processStep >= phase.length - 1}
            onClick={() =>
              setProcessStep((current) =>
                Math.min(phase.length - 1, current + 1),
              )
            }
          >
            查看下一步 →
      ...
      <div className="compact-evidence-row">
        <section className="card compact-messages">
          <div>
            <span>本轮消息</span>
            <small>始终可见 · 内容未验证</small>
          </div>
        ...
        <section className="card compact-opponent-view">
          <div>
            <span>对手判断</span>
            <small>只依据已经结算的公开历史</small>
          </div>
          {!hasHistoricalEvidence ? (
            <p className="compact-empty">第一回合尚无历史行为，暂不判断。</p>
          ) : (
            human.beliefs.map((belief) => (
              <article key={belief.companyId}>
                <b>{belief.companyId.slice(-1)}</b>
                <span>降价可能性</span>
                <strong>{belief.nextAction.cut}%</strong>
      ...
      <div className="live-main-action-grid">
        {interactive ? (
          <section className="card live-action-panel primary-task">
            <div className="mini-panel-head">
              <span>你的本轮决策</span>
              <b>{completed ? "已锁定" : "可以修改"}</b>
            </div>
            <h3>第 {round} 回合动作</h3>
            <div className="live-action-controls">
              <label>
                <span>
                  价格 <b>{formatMoney(humanAction.price)}</b>
                </span>
                <small>价格更低通常有利于份额，但会压缩每笔订单利润。</small>
                <input
                  disabled={completed}
                  type="range"
                  min={8000}
                  max={12000}
                  step={50}
                  value={humanAction.price}
                  onChange={(event) =>
                    setHumanAction((current) => ({
                      ...current,
              ...
              <label>
                <span>
                  广告预算 <b>{formatMoney(humanAction.advertising, true)}</b>
                </span>
                <small>用于吸引订单，当期需要支付成本。</small>
                <input
                  disabled={completed}
                  type="range"
                  min={0}
                  max={2000000}
                  step={100000}
                  value={humanAction.advertising}
                  onChange={(event) =>
                    setHumanAction((current) => ({
                      ...current,
                      advertising: Number(event.target.value),
                    }))
                  }
                />
              </label>
              <label>
                <span>
                  共享抗冲击投入{" "}
                  <b>{formatMoney(humanAction.contribution, true)}</b>
                </span>
                <small>当期支付成本，未来全行业都可能获得保护。</small>
                <input
                  disabled={completed}
                  type="range"
                  min={0}
                  max={2000000}
                  step={100000}
                  value={humanAction.contribution}
                  onChange={(event) =>
                    setHumanAction((current) => ({
                      ...current,
            ...
            <button
              ...
              type="button"
              disabled={completed || busy}
              onClick={() => autoRun(humanAction)}
            >
              {busy
                ? "协调器结算中…"
                : runtimeMode === "backend" && needsCoordinator
                  ? "协调器跑完剩余轮次"
                  : "规则代理跑完剩余轮次"}
            </button>
          </section>
        ) : (
          <section className="card observer-round-panel primary-task">
            <div className="mini-panel-head">
              <span>观察推进</span>
              <b>不控制任何公司</b>
            </div>
            ...
            <button
              ...
              onClick={moveToNextRound}
            >
              {round === maxRounds ? "观察最终回合结算" : "观察下一回合"}
              <i>→</i>
            </button>
            <button
              className="ghost-button"
        ...
        <section className="card live-settlement-panel">
          <div className="mini-panel-head">
            <span>最近一次市场结算</span>
            <b>{hasSettledRound ? "已有结果" : "等待第一轮"}</b>
          </div>
          <h3>
            {hasSettledRound
              ? `第 ${Math.max(1, round - (completed ? 0 : 1))} 回合结果`
              : "尚无已结算回合"}
          </h3>
          <div className="settlement-table">
            ...
            {agents.map((agent, index) => (
              <article key={agent.companyId}>
                <b style={{ color: agent.color }}>
                  {agent.companyId.slice(-1)}
                </b>
                <span>{formatMoney(agent.action.price)}</span>
                <em className={agent.shareDelta >= 0 ? "up" : "down"}>
                  {agent.shareDelta === 0
                    ? "—"
                    : `${agent.shareDelta > 0 ? "+" : ""}${agent.shareDelta.toFixed(1)} 个百分点`}
                ...
                <strong>
                  {index === 0 ? formatMoney(agent.profit, true) : "未公开"}
                </strong>
              </article>
            ))}
          </div>
          <p className="settlement-proof">
            全部公司市场份额合计为 <b>{shareTotal.toFixed(1)}%</b>
            。对手利润属于私有信息，因此不向参与者展示。
          </p>
        </section>
      </div>
    </div>
...
function ObservatoryView({ agents }: { agents: AgentRuntimeView[] }) {
  ...
  const agent =
    agents.find((item) => item.companyId === companyId) ?? agents[0];
  return (
    <div className="view-pad observatory-view">
      <section className="observatory-header card">
        <div>
          <span>智能体范围审计</span>
          <h2>这个智能体在这一轮究竟知道什么？</h2>
          <p>
            可见信息、自己的私有信息、隐藏信息和概率判断被严格分开。这里不展示模型隐藏思维过程。
          </p>
        </div>
        <div className="agent-selector">
          {agents.map((item) => (
            <button
              type="button"
              className={item.companyId === companyId ? "active" : ""}
              style={{ "--agent": item.color } as React.CSSProperties}
              key={item.companyId}
              onClick={() => setCompanyId(item.companyId)}
            >
              <i>{item.companyId.slice(-1)}</i>
              <span>
                {item.companyName}
      ...
      <div className="observatory-layout">
        <aside className="agent-dossier card">
          <div className="dossier-avatar" style={{ background: agent.color }}>
            {agent.companyId.slice(-1)}
          </div>
          <span>当前智能体</span>
          ...
          <dl>
            ...
            <div>
              <dt>输入状态</dt>
              <dd className="good">已冻结</dd>
            </div>
          </dl>
          <div className="privacy-rule">
            <b>信息边界</b>
            <p>对手现金、利润、成本、事故细节和真实人格不可见。</p>
          </div>
        </aside>
        <section className="cognition-panel card">
          <div className="tabbar">
            {(["observation", "belief", "planning", "decision"] as const).map(
              (item) => (
                <button
                  type="button"
          ...
          {tab === "observation" && (
            <div className="observation-grid">
              <div className="visibility-column visible">
                <h3>
                  <i>✓</i> 公共状态 <span>所有公司可见</span>
                </h3>
                {agent.observation.public.map((item) => (
                  <article key={item.label}>
                    <span>{item.label}</span>
              ...
              <div className="visibility-column private">
                <h3>
                  <i>●</i> 自己的私有状态 <span>仅自己可见</span>
                </h3>
                {agent.observation.private.map((item) => (
                  <article key={item.label}>
                    <span>{item.label}</span>
                    <strong>{item.value}</strong>
                  </article>
              ...
              <div className="visibility-column hidden">
                <h3>
                  <i>⌁</i> 对手状态 <span>已隐藏</span>
                </h3>
                {agent.observation.hidden.map((item) => (
                  <article key={item.label}>
                    <span>{item.label}</span>
                    <strong>{item.value}</strong>
                  </article>
              ...
              <div className="untrusted-note">
                <b>未验证消息边界</b>
                <p>
                  通信中的“现金充足”“准备扩产”等声明只作为未验证信号进入对手判断，绝不会写回事实状态。
                </p>
              </div>
            </div>
          )}
              ...
              {agent.beliefs.map((belief) => (
                <article key={belief.companyId}>
                  <div className="belief-head">
                    <span>{belief.companyId.slice(-1)}</span>
                    <div>
                      <strong>{belief.companyId}</strong>
                      <small>基于公开证据的概率判断</small>
                    </div>
                  </div>
                  <h4>策略倾向</h4>
                  {Object.entries(belief.strategy).map(([key, value]) => (
                    <div className="belief-bar" key={key}>
                  ...
                  <h4>下一轮价格动作</h4>
                  {Object.entries(belief.nextAction).map(([key, value]) => (
                    <div className="belief-bar compact" key={key}>
                  ...
                  <details>
                    <summary>判断依据 · {belief.evidence.length} 条</summary>
                    <ul>
          ...
          {tab === "planning" && (
            <div className="planning-view">
              <div className="plan-hero">
                <span>当前目标</span>
                <h3>{agent.plan.goal}</h3>
                <p>计划考虑未来 {agent.plan.horizon} 个回合</p>
              </div>
              <div className="plan-steps">
                {agent.plan.subgoals.map((goal, index) => (
                  <article className={goal.status} key={goal.label}>
                    <i>{goal.status === "done" ? "✓" : index + 1}</i>
                    <span>
                      {goal.label}
                      <small>{PLAN_STATUS_LABELS[goal.status]}</small>
                    </span>
                  </article>
                ))}
              </div>
              <div className="replan-triggers">
                <span>重新规划条件</span>
                {agent.plan.triggers.map((trigger) => (
          ...
          {tab === "decision" && (
            <div className="decision-summary-view">
              <div className="no-cot">
                <span>可审计决策摘要</span>
                <b>不展示隐藏思维过程</b>
              </div>
              <article>
                <span>当前局面</span>
                <p>{agent.decision.situation}</p>
              </article>
              <article>
                <span>主要依据</span>
                <ol>
                  {agent.decision.factors.map((factor) => (
              ...
              <article>
                <span>最终决定</span>
                <p className="decision-highlight">{agent.decision.summary}</p>
              </article>
              <article>
                <span>预期结果</span>
                <p>{agent.decision.expected}</p>
              </article>
            </div>
          )}
        </section>
...
function CommunicationView() {
  ...
  const messages = DEMO_MESSAGES.filter(
    (message) => filter === "all" || message.channel === filter,
  );
  ...
  return (
    <div className="view-pad communication-view">
      <SectionHead
        eyebrow="交流、提议与承诺"
        title="通信记录"
        description="公开消息、私信、提议与履约结果位于同一条时间线。所有消息都不具有强制约束力，也不一定真实。"
        action={
      ...
      <div className="communication-layout">
        <section className="message-stream card">
          <div className="stream-head">
            <span>第 5 回合 · 通信已关闭</span>
            <b>{messages.length} 条消息</b>
          </div>
          {messages.map((message) => (
            <article className={message.channel} key={message.id}>
              <div className="message-route">
                <i>{message.sender.slice(-1)}</i>
                <span>
                  {message.sender}
                  {message.recipient
                    ? ` → ${message.recipient}`
                    : " → 所有公司"}
                  <small>
                    {message.channel === "public" ? "公开" : "私信"} ·{" "}
                    {kindLabels[message.kind]}
                  </small>
                </span>
                <time>第 {message.round} 回合</time>
              </div>
              <blockquote>{message.text}</blockquote>
              <div className="message-meta">
                <span>可见公司：{message.visibility.join("、")}</span>
                {message.status && (
        ...
        <aside className="communication-audit card">
          <span>消息可见范围</span>
          <h3>谁能看见什么</h3>
          <div className="matrix">
            ...
            {DEMO_MESSAGES.slice(0, 4).map((message) => (
              <div key={message.id}>
                <span>
                  {message.sender.slice(-1)}
                  {message.recipient
                    ? `→${message.recipient.slice(-1)}`
                    : "→全部"}
                </span>
                {["A", "B", "C", "D"].map((company) => (
                  <i
                    className={
                      message.visibility.includes(company) ? "seen" : "hidden"
                    }
                    key={company}
                  >
                    {message.visibility.includes(company) ? "✓" : "–"}
                  </i>
                ))}
              </div>
            ))}
          ...
          <div className="audit-callout">
            <b>未发现可见性泄漏</b>
            <p>非接收者看不到私信的编号、数量、时间或正文。</p>
          </div>
          <div className="commitment-chain">
            <span>承诺履约过程</span>
            <ol>
              <li>
                <i>01</i>提出投入 10,000 元
              </li>
              <li>
                <i>02</i>C 接受提议
              </li>
...
function StrategyView({ agents }: { agents: AgentRuntimeView[] }) {
  ...
  const agent =
    agents.find((item) => item.companyId === companyId) ?? agents[1];
  const pipeline = [
    "可见信息",
    "动作概率判断",
    "对手策略判断",
  ...
  return (
    <div className="view-pad strategy-view">
      <SectionHead
        eyebrow="对手判断 → 效用推断 → 策略建议"
        title="博弈策略分析"
        description="每一层都记录输入来源；策略建议不具有强制约束力，最终决定仍由智能体提交。"
        action={
          <select
            value={companyId}
      ...
      />
      <section className="strategy-pipeline">
        {pipeline.map((item, index) => (
          <div key={item} className={index === 4 ? "focus" : ""}>
            <i>{index + 1}</i>
            <span>
              {item}
              <small>
      ...
      <div className="strategy-grid">
        <section className="card opponent-model-card">
          <span>对手策略判断</span>
          <h3>对手策略倾向</h3>
          {agent.beliefs.map((belief) => (
            <article key={belief.companyId}>
              <div>
                <b>{belief.companyId}</b>
                <small>{belief.evidence.length} 条公开依据</small>
              </div>
              <div
                className="strategy-stack"
                title="增长导向、利润导向、防御导向"
              >
                <i style={{ width: `${belief.strategy.growth}%` }} />
                <i style={{ width: `${belief.strategy.profit}%` }} />
                <i style={{ width: `${belief.strategy.defensive}%` }} />
              </div>
              <em>增长导向 {belief.strategy.growth}%</em>
            </article>
          ))}
        ...
        <section className="card utility-card">
          <span>效用目标推断</span>
          <h3>推断偏好，不等于真实人格</h3>
          <div className="utility-radar">
            <svg viewBox="0 0 220 180" role="img" aria-label="效用权重雷达图">
              <polygon
                points="110,12 204,72 168,168 52,168 16,72"
                className="radar-grid"
              />
              <polygon
                points={`110,${100 - agent.utility.profit} ${110 + agent.utility.growth * 1.5},78 ${145 + agent.utility.risk / 2},142 66,142 42,78`}
          ...
          <div className="utility-values">
            <span>
              利润 <b>{agent.utility.profit}%</b>
            </span>
            <span>
              增长 <b>{agent.utility.growth}%</b>
            </span>
            <span>
              风险 <b>{agent.utility.risk}%</b>
            </span>
          </div>
        </section>
        <section className="card advisor-card">
          ...
          <div className="candidate-table">
            ...
            {agent.advisor.candidates.map((candidate, index) => (
              <article
              ...
              >
                ...
                <strong>{candidate.utility.toFixed(2)}</strong>
                <em>{candidate.risk.toFixed(2)}</em>
              </article>
            ))}
          </div>
          <div className="advisor-result">
            <span>
              建议价格
              <strong>{formatMoney(agent.advisor.recommendedPrice)}</strong>
            </span>
            <i>→</i>
            <span>
              最终价格<strong>{formatMoney(agent.action.price)}</strong>
            </span>
            <b className={agent.advisor.adopted ? "adopted" : "rejected"}>
              {agent.advisor.adopted ? "已采纳" : "未采纳"}
            </b>
          </div>
          <p>
            是否有效必须使用真实市场结果评价；内部评分不能证明建议提高了利润。
          </p>
        </section>
      </div>
    </div>
...
function LineChart() {
  ...
  const path = (key: (typeof keys)[number]) =>
    PROFIT_SERIES.map(
      (point, index) =>
        `${index ? "L" : "M"}${30 + index * 112},${190 - point[key] * 52}`,
    ).join(" ");
  return (
    <svg
    ...
    >
      <g className="chart-grid">
        {[30, 80, 130, 180].map((y) => (
          <line key={y} x1="28" y1={y} x2="485" y2={y} />
        ))}
      </g>
      {keys.map((key) => (
        <g key={key}>
...
function MarketView({
  ...
  const gradients = agents
    .map(
      (agent, index) =>
        `${agent.color} ${agents.slice(0, index).reduce((sum, item) => sum + item.share, 0)}% ${agents.slice(0, index + 1).reduce((sum, item) => sum + item.share, 0)}%`,
    )
    .join(",");
  return (
    <div className="view-pad market-view">
      <SectionHead
        eyebrow="市场结果与公共状态"
        title="市场面板"
      ...
      <div className="market-grid">
        <section className="card chart-panel">
          ...
          <div className="chart-axis">
            {PROFIT_SERIES.map((point) => (
              <span key={point.round}>第 {point.round} 轮</span>
            ))}
          </div>
        </section>
          ...
          <div className="investment-table">
            ...
            {agents.map((agent) => (
              <article key={agent.companyId}>
                ...
                {[
                  agent.action.advertising,
                  agent.action.service,
                  agent.action.capacity,
                  agent.action.resilience,
                ].map((value, index) => (
                  <span key={index}>
                    <i style={{ width: `${Math.min(100, value / 8000)}%` }} />
                    <em>{formatMoney(value, true)}</em>
                  </span>
                ))}
              </article>
            ))}
...
function ReplayView() {
  ...
  return (
    <div className="view-pad replay-view">
      <SectionHead
        eyebrow="确定性输入重建"
        title="回合重建"
        description="按照市场状态 → 可见信息 → 对手判断 → 通信 → 决策 → 动作 → 结果重建过程；不会重新调用模型生成语义。"
        action={
          <div className="replay-selectors">
            <select aria-label="实验">
              <option>第 1001 号随机种子实验</option>
            </select>
            <select aria-label="回合">
              <option>第 5 回合</option>
            </select>
            <select aria-label="智能体">
              <option>全部智能体</option>
            </select>
          </div>
        }
      ...
      <div className="replay-layout">
        <section className="replay-timeline card">
          {DEMO_REPLAY.map((item, index) => (
            <button
              type="button"
              key={`${item.phase}-${index}`}
              className={`${item.tone}${selected === index ? " selected" : ""}`}
              onClick={() => setSelected(index)}
            >
              <i>{index + 1}</i>
              <div>
                <span>
                  {item.phase} · {item.agent}
                </span>
        ...
        <section className="replay-inspector card">
          ...
          <div className="json-view">
            ...
            <p>
              <i>"phase"</i>:{" "}
              <em>"{step.phase.toLowerCase().replace(" ", "_")}"</em>,
            </p>
            <p>
              <i>"company_scope"</i>: <em>"{step.agent}"</em>,
            </p>
            <p>
              <i>"source_hash"</i>: <em>"{step.hash ?? "derived"}"</em>,
            </p>
            <p>
              <i>"replay_match"</i>: <b>true</b>
            </p>
            <span>{`}`}</span>
          </div>
...
function ResearchArchiveView({ controllerToken }: { controllerToken: string }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/v1/research/experiments`)
      .then(async (response) => {
      ...
      .then((payload) => {
        ...
        if (first) {
          ...
          setCompanyId(
            first.company_ids.includes("company_B")
              ? "company_B"
              : first.company_ids[0],
          );
        }
      ...
      .catch((reason) =>
        setError(
          `真实档案加载失败：${reason instanceof Error ? reason.message : "未知错误"}`,
        ),
      );
  }, []);
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch(`${API_URL}/v1/research/experiments/${experimentId}/manifest`).then(
        (response) => response.json(),
      ),
      fetch(
        `${API_URL}/v1/research/experiments/${experimentId}/integrity`,
  ...
  useEffect(() => {
    ...
    fetch(
      `${API_URL}/v1/research/experiments/${experimentId}/rounds/${roundNumber}/agents/${companyId}?view=${viewMode}`,
      {
        headers:
          viewMode === "authority"
      ...
      .catch((reason) => {
        setTrace(null);
        setError(
          `回合档案加载失败：${reason instanceof Error ? reason.message : "未知错误"}`,
        );
      });
  ...
  const chooseExperiment = (nextId: string) => {
    ...
    if (next) {
      ...
      setCompanyId(
        next.company_ids.includes("company_B")
          ? "company_B"
          : next.company_ids[0],
      );
    }
  ...
  const traceView = trace as {
    observation_hash?: string;
    advisor?: { advice_hash?: string };
    replay_status?: Record<string, string>;
    final_action?: Record<string, unknown>;
    agent_version?: Record<string, unknown>;
  } | null;
  return (
    <div className="view-pad archive-view">
      <SectionHead
        eyebrow="冻结的研究证据"
        title="真实实验只读档案"
        description="这里读取发布清单登记的真实或工程 Episode，不重新调用模型，也不允许输入任意文件路径。"
        action={
          <span className="real-badge">
            {selected?.evidence_type ?? "档案"} · 只读
          </span>
        }
      />
      <section className="archive-controls card">
        ...
        <label>
          ...
          <select
            ...
            onChange={(event) => setCompanyId(event.target.value)}
          >
            {(selected?.company_ids ?? []).map((item) => (
              <option key={item}>{item}</option>
            ))}
          </select>
        </label>
        <div className="archive-view-switch">
          <button
            type="button"
      ...
      <div className="archive-grid">
        <aside className="card archive-evidence">
          <span>证据边界</span>
          <h3>{selected?.title ?? "正在加载"}</h3>
          <p>{selected?.claim_boundary}</p>
          <dl>
            <div>
              <dt>证据类型</dt>
            ...
            <div>
              <dt>完整性</dt>
              <dd>
                {(integrity as { artifact_integrity_passed?: boolean } | null)
            ...
            <div>
              <dt>观察校验值</dt>
              <dd>{traceView?.observation_hash ?? "—"}</dd>
            </div>
            <div>
              <dt>建议校验值</dt>
              <dd>{traceView?.advisor?.advice_hash ?? "无建议或审计视角"}</dd>
            </div>
          </dl>
          <div className="archive-boundary">
            <b>视角隔离</b>
            <p>
              智能体视角只返回它当时收到的
              Observation、可见消息、判断、建议和动作；完整权威状态必须经过
              Controller 授权。
            </p>
          </div>
        </aside>
        <section className="card archive-json">
          <div>
            <span>逐轮证据</span>
            <b>{viewMode === "agent" ? "智能体当时视角" : "权威审计视角"}</b>
          </div>
          <pre>
            {authorityBlocked
              ? "权威审计视角已锁定。"
      ...
      <section className="archive-hashline card">
        <span>Agent Version</span>
        <code>
          {JSON.stringify(traceView?.agent_version ?? manifest?.agents ?? {})}
        </code>
        <span>Replay</span>
        <code>{JSON.stringify(traceView?.replay_status ?? {})}</code>
      </section>
    </div>
  );
}
...
function ReportView({
  ...
  const averageResilience =
    agents.reduce((sum, agent) => sum + agent.resilience, 0) / agents.length;
  const totalShare = agents.reduce((sum, agent) => sum + agent.share, 0);
  return (
    <div className="view-pad report-view">
      <section className="report-hero">
        <div>
          <span>实验已完成 · 最终总结</span>
          <h2>本次实验已经完成。</h2>
          <p>
            {runtimeMode === "demo"
              ? "这是交互演示的回合总结，不是大模型实验结论。"
              : ruleAutoRunUsed
                ? "剩余回合由确定性规则代理完成，不代表原配置中的大模型继续做出了决策。"
                : "报告来自当前后端实验；研究结论仍需使用共同随机种子进行对照。"}
          </p>
        </div>
        <button type="button">
          导出当前结果 <i>↗</i>
      ...
      <div className="report-scorecard">
        <article>
          <span>利润最高</span>
          <strong style={{ color: profitWinner.color }}>
            {profitWinner.companyName}
          </strong>
          <small>{formatMoney(profitWinner.profit)}</small>
        </article>
        <article>
          <span>市场份额最高</span>
          <strong style={{ color: shareWinner.color }}>
            {shareWinner.companyName}
          </strong>
          <small>{formatPercent(shareWinner.share)}</small>
        </article>
        <article>
          <span>行业平均抗冲击能力</span>
          <strong>{averageResilience.toFixed(1)}%</strong>
          <small>{agents.length} 家公司平均值</small>
        </article>
        <article>
          <span>市场份额合计</span>
          <strong>{totalShare.toFixed(1)}%</strong>
          <small>必须等于 100%</small>
        </article>
      </div>
      <div className="report-grid clean-report">
        <section className="card report-findings">
          ...
          <article>
            <i>01</i>
            <div>
              <strong>市场竞争已经完成</strong>
              <p>价格和投入共同影响份额与利润，全部公司份额总和保持 100%。</p>
            </div>
            <b>已观察到</b>
          </article>
          <article>
            <i>02</i>
            <div>
              <strong>份额不等于利润</strong>
              <p>
                份额领先者与利润领先者可能不是同一家公司，需要同时解释竞争和经营结果。
              </p>
            </div>
            <b>描述性结果</b>
          </article>
          <article className="warn">
            ...
            <div>
              <strong>
                {runtimeMode === "demo" || ruleAutoRunUsed
                  ? "不能作为真实模型行为证据"
                  : "单个实验不能形成因果结论"}
              </strong>
              <p>
                {runtimeMode === "demo"
                  ? "正式结论必须来自后端市场环境、真实智能体、回合事件和过程重建。"
                  : ruleAutoRunUsed
                    ? "规则续跑只验证市场可结算性；研究真实智能体时必须由 Coordinator 完整推进。"
                    : "至少需要共同随机种子、固定处理条件和配对统计。"}
              </p>
            </div>
            <b>证据边界</b>
          </article>
        </section>
        <section className="card agent-report">
          <span>智能体最终结果</span>
          <h3>公司结果</h3>
          {agents.map((agent) => (
            <article key={agent.companyId}>
              ...
              <div>
                ...
                <p>{agent.decision.summary}</p>
              </div>
              <span>
                <b>{formatPercent(agent.share)}</b>
                <small>市场份额</small>
              </span>
              <span>
                <b>{formatMoney(agent.profit, true)}</b>
                <small>利润</small>
              </span>
            </article>
          ))}
...
function hydrateAgents(payload: BackendEpisode, configs: AgentConfig[]) {
  return buildRuntimeAgents(configs).map((agent) => {
...
function hydrateDemoAgents(configs: AgentConfig[]) {
  return buildRuntimeAgents(configs).map((agent) => {
    ...
    return {
      ...
      action: { ...agent.action },
      observation: {
        public: [...agent.observation.public],
        private: [...agent.observation.private],
        hidden: [...agent.observation.hidden],
      },
    };
  });
}
...
export default function Home() {
  ...
  const [personaOptions, setPersonaOptions] = useState<PersonaKey[]>(
  ...
  const marketModel = useMemo(
    () =>
      (
        ({
          balanced: "balanced",
          high_demand: "quality_oriented",
          supply_crisis: "value_oriented",
          disaster: "service_oriented",
          public_goods: "balanced",
        }) as const
      )[config.marketType],
    [config.marketType],
  );
  ...
  function apiHeaders() {
    return {
      ...
      ...(config.controllerToken
        ? { "X-Controller-Token": config.controllerToken }
        : {}),
    };
  }
    ...
    setRuntimeAgents((current) =>
      hydrateAgents(payload, agents).map((agent) => {
        const previous = current.find(
          (item) => item.companyId === agent.companyId,
        );
        const shareDelta = previous
          ? Math.round((agent.share - previous.share) * 10) / 10
          : 0;
        if (
          humanAction &&
          agent.companyId ===
            (agents.find((item) => item.driver === "human")?.companyId ??
              agents[0]?.companyId)
        ) {
          return {
            ...agent,
  ...
  async function startExperiment(forceDemo: boolean) {
    ...
    if (
      !personaCatalogVerified ||
      agents.some((agent) => !personaOptions.includes(agent.persona))
    ) {
      setNotice("真实实验已阻止：必须先从后端已验收人格目录选择全部人格。");
      return;
    }
    const protectedMode =
      config.communication ||
    ...
    try {
      const response = await fetch(`${API_URL}/episodes`, {
        ...
        body: JSON.stringify({
          ...
          company_ids: agents.map((agent) => agent.companyId),
          personas: Object.fromEntries(
            agents.map((agent) => [
              agent.companyId,
              agent.persona.startsWith("aggressive")
                ? "aggressive"
                : agent.persona.startsWith("risk")
                  ? "conservative"
                  : "balanced",
            ]),
          ),
          agent_configs: Object.fromEntries(
            agents.map((agent) => [
              agent.companyId,
              {
                agent_id: `${agent.driver}-${agent.companyId}`,
                agent_type:
                  agent.driver === "rule"
                    ? "rule"
                    : agent.driver === "human"
          ...
          game_mode:
            ...
            !needsCoordinator
              ...
              : "market",
          player_company_id:
            agents.find((agent) => agent.driver === "human")?.companyId ?? null,
  ...
  function nextRound(humanAction: HumanAction) {
    ...
    setRuntimeAgents((current) =>
      advanceDemoRound(current, settledRound, humanAction),
    );
    if (settledRound >= config.rounds) {
      setDemoCompleted(true);
      setNotice(
        `第 ${settledRound} 回合已结算，Episode 完成；不会循环回第 1 回合。`,
      );
      return;
    }
    setRound(settledRound + 1);
    setNotice(
      `第 ${settledRound} 回合已联合结算；现在进入第 ${settledRound + 1} 回合决策。演示结果不作为研究证据。`,
    );
  }
  ...
  async function autoRunRemaining(humanAction: HumanAction) {
    ...
    if (runtimeMode === "demo") {
      ...
      let settledRound = round;
      while (settledRound <= config.rounds) {
        current = advanceDemoRound(current, settledRound, humanAction);
        settledRound += 1;
      }
      setRuntimeAgents(current);
      setRound(config.rounds);
      setDemoCompleted(true);
      setRuleAutoRunUsed(true);
      setNotice("规则代理已跑完剩余回合；演示结果不作为研究证据。");
      setActive("report");
      return;
    }
  ...
  async function advanceBackendRound(
    ...
    try {
      const stateResponse = await fetch(
        `${API_URL}/episodes/${episodeId}/state`,
        { headers: apiHeaders() },
      );
      const current = (await stateResponse.json()) as BackendEpisode & {
        detail?: string;
      };
      if (!stateResponse.ok)
        throw new Error(
      ...
      const playerAction = hasHuman
        ? playerActionPayload(current.state, humanAction)
        : null;
      if (!remaining && !needsCoordinator && hasHuman) {
        const response = await fetch(
          `${API_URL}/episodes/${episodeId}/player-steps`,
          {
            method: "POST",
            headers: apiHeaders(),
            body: JSON.stringify({
              step_id: `${current.state.episode_id}:${current.state.round}:${current.state.state_version ?? 0}`,
        ...
        setNotice(
          terminal
            ? `第 ${config.rounds} 回合已结算，Episode 完成。`
            : "后端已结算当前回合。",
        );
        return;
      }
      ...
      if (
        ...
        setNotice("已取消真实模型调用，市场状态未改变。");
        return;
      }
      const response = await fetch(
        `${API_URL}/v1/controller/episodes/${episodeId}/coordinator-run`,
        {
          method: "POST",
          headers: apiHeaders(),
          body: JSON.stringify({
      ...
      const terminal = applyBackendPayload(
        payload,
        hasHuman ? humanAction : undefined,
      );
      setRuleAutoRunUsed(false);
      if (remaining) {
        setNotice(
      ...
      setNotice(
        error instanceof Error
          ? `${remaining ? "自动跑完" : "推进回合"}失败：${error.message}`
          : "推进失败；状态未改变。",
      );
    } finally {
      setBusy(false);
  ...
  const view = theoryLab ? <TheoryLab apiUrl={API_URL} token={config.controllerToken} episodeId={runtimeMode==='backend'?episodeId:''} onWorkbench={()=>{setTheoryLab(false);setWorkbench(true);}}/> : workbench ? <ResearchWorkbench apiUrl={API_URL} token={config.controllerToken}/> :
    active === "home" ? (
      <LandingView
        choose={chooseEntry}
        resume={() => setActive("live")}
        hasSession={Boolean(episodeId)}
      />
    ) : active === "setup" ? (
      <SetupView
        config={config}
        ...
        personaCatalogVerified={personaCatalogVerified}
      />
    ) : active === "archive" ? (
      <ResearchArchiveView controllerToken={config.controllerToken} />
    ) : active === "live" ? (
      <LiveView
        ...
        nextRound={nextRound}
        autoRun={(action) => void autoRunRemaining(action)}
        runtimeMode={runtimeMode}
        notice={notice}
  ...
  return (
    ...
    >
      ...
      {backendConfig?.local_model_budget && <section className="card local-run-control" aria-label="真实模型费用"><p>真实模型总上限 ¥{backendConfig.local_model_budget.total_cny} · 已保留 ¥{backendConfig.local_model_budget.reserved_cny?.toFixed(3) ?? "待核对"} · 可用 ¥{backendConfig.local_model_budget.remaining_cny?.toFixed(3) ?? "0"}。规则模拟免费；已接入 DeepSeek Flash 与豆包 Lite。完整上下文每次最多保留 ¥0.42，工作台紧凑决策按请求上界保留，失败不自动退还额度。{backendConfig.local_model_budget.message}</p></section>}
