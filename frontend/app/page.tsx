...
function SetupView({ config, setConfig, agents, setAgents, editingAgent, setEditingAgent, start, busy, notice, backendOnline, entryMode, personaOptions, personaLabels, personaCatalogVerified }: { config: LabConfig; setConfig: React.Dispatch<React.SetStateAction<LabConfig>>; agents: AgentConfig[]; setAgents: React.Dispatch<React.SetStateAction<AgentConfig[]>>; editingAgent: string | null; setEditingAgent: (id: string | null) => void; start: (forceDemo: boolean) => void; busy: boolean; notice: string; backendOnline: boolean | null; entryMode: EntryMode; personaOptions: PersonaKey[]; personaLabels: Record<string, string>; personaCatalogVerified: boolean }) {
  ...
  function addAgent() { if (agents.length < MAX_COMPANIES) setAgents((current) => [...current, { ...AGENT_TEMPLATES[current.length] }]); }
...
function LiveView({ agents, round, maxRounds, nextRound, autoRun, runtimeMode, notice, completed, interactive, busy }: { agents: AgentRuntimeView[]; round: number; maxRounds: number; nextRound: (action: { price: number; advertising: number; contribution: number }) => void; autoRun: (action: { price: number; advertising: number; contribution: number }) => void; runtimeMode: RuntimeMode; notice: string; completed: boolean; interactive: boolean; busy: boolean }) {
  ...
  return <div className="view-pad live-view"><section className="live-command card"><div className="round-orbit"><span>当前决策回合</span><strong>{round}</strong><small>/ {maxRounds}</small><i style={{ "--progress": `${maxRounds === 1 ? 100 : Math.min(100, ((round - 1) / (maxRounds - 1)) * 100)}%` } as React.CSSProperties} /></div><div className="phase-track">{phase.map((item, index) => <div className={completed || index < processStep ? "done" : index === processStep ? "active" : ""} key={item}><i>{completed || index < processStep ? "✓" : index + 1}</i><span>{item}</span></div>)}</div><div className="live-actions"><span>{runtimeMode === "backend" ? "后端真实回合" : "前端交互演示"}<small>{notice}</small></span><b className={completed ? "round-complete" : "round-open"}>{completed ? "本次体验已完成" : `第 ${round} 回合进行中`}</b></div></section>
    ...
    <div className="live-main-action-grid">{interactive ? <section className="card live-action-panel primary-task"><div className="mini-panel-head"><span>你的本轮决策</span><b>{completed ? "已锁定" : "可以修改"}</b></div><h3>第 {round} 回合动作</h3><div className="live-action-controls"><label><span>价格 <b>{formatMoney(humanAction.price)}</b></span><small>价格更低通常有利于份额，但会压缩每笔订单利润。</small><input disabled={completed} type="range" min={8000} max={12000} step={50} value={humanAction.price} onChange={(event) => setHumanAction((current) => ({ ...current, price: Number(event.target.value) }))} /></label><label><span>广告预算 <b>{formatMoney(humanAction.advertising, true)}</b></span><small>用于吸引订单，当期需要支付成本。</small><input disabled={completed} type="range" min={0} max={2000000} step={100000} value={humanAction.advertising} onChange={(event) => setHumanAction((current) => ({ ...current, advertising: Number(event.target.value) }))} /></label><label><span>共享抗冲击投入 <b>{formatMoney(humanAction.contribution, true)}</b></span><small>当期支付成本，未来全行业都可能获得保护。</small><input disabled={completed} type="range" min={0} max={2000000} step={100000} value={humanAction.contribution} onChange={(event) => setHumanAction((current) => ({ ...current, contribution: Number(event.target.value) }))} /></label></div><button className="settle-round-button" type="button" disabled={completed || busy} onClick={moveToNextRound}>{runtimeMode === "backend" ? "请求后端推进回合" : round === maxRounds ? "提交并结算最终回合" : `提交第 ${round} 回合并进入下一轮`}<i>→</i></button><button className="ghost-button" type="button" disabled={completed || busy} onClick={() => autoRun(humanAction)}>{busy ? "规则代理结算中…" : "规则代理跑完剩余轮次"}</button></section> : <section className="card observer-round-panel primary-task"><div className="mini-panel-head"><span>观察推进</span><b>不控制任何公司</b></div><h3>观察第 {round} 回合</h3><p>系统使用 {agents.length} 个智能体各自的策略完成联合动作与市场结算。</p><button className="settle-round-button" type="button" disabled={completed || busy} onClick={moveToNextRound}>{round === maxRounds ? "观察最终回合结算" : "观察下一回合"}<i>→</i></button><button className="ghost-button" type="button" disabled={completed || busy} onClick={() => autoRun(humanAction)}>{busy ? "规则代理结算中…" : "规则代理跑完剩余轮次"}</button></section>}
      <section className="card live-settlement-panel"><div className="mini-panel-head"><span>最近一次市场结算</span><b>{hasSettledRound ? "已有结果" : "等待第一轮"}</b></div><h3>{hasSettledRound ? `第 ${Math.max(1, round - (completed ? 0 : 1))} 回合结果` : "尚无已结算回合"}</h3><div className="settlement-table"><div><span>公司</span><span>价格</span><span>份额变化</span><span>利润</span></div>{agents.map((agent, index) => <article key={agent.companyId}><b style={{ color: agent.color }}>{agent.companyId.slice(-1)}</b><span>{formatMoney(agent.action.price)}</span><em className={agent.shareDelta >= 0 ? "up" : "down"}>{agent.shareDelta === 0 ? "—" : `${agent.shareDelta > 0 ? "+" : ""}${agent.shareDelta.toFixed(1)} 个百分点`}</em><strong>{index === 0 ? formatMoney(agent.profit, true) : "未公开"}</strong></article>)}</div><p className="settlement-proof">全部公司市场份额合计为 <b>{shareTotal.toFixed(1)}%</b>。对手利润属于私有信息，因此不向参与者展示。</p></section></div>
  </div>;
}
...
function ReportView({ agents, runtimeMode, ruleAutoRunUsed }: { agents: AgentRuntimeView[]; runtimeMode: RuntimeMode; ruleAutoRunUsed: boolean }) {
  ...
  return <div className="view-pad report-view"><section className="report-hero"><div><span>实验已完成 · 最终总结</span><h2>本次实验已经完成。</h2><p>{runtimeMode === "demo" ? "这是交互演示的回合总结，不是大模型实验结论。" : ruleAutoRunUsed ? "剩余回合由确定性规则代理完成，不代表原配置中的大模型继续做出了决策。" : "报告来自当前后端实验；研究结论仍需使用共同随机种子进行对照。"}</p></div><button type="button">导出当前结果 <i>↗</i></button></section><div className="report-scorecard"><article><span>利润最高</span><strong style={{ color: profitWinner.color }}>{profitWinner.companyName}</strong><small>{formatMoney(profitWinner.profit)}</small></article><article><span>市场份额最高</span><strong style={{ color: shareWinner.color }}>{shareWinner.companyName}</strong><small>{formatPercent(shareWinner.share)}</small></article><article><span>行业平均抗冲击能力</span><strong>{averageResilience.toFixed(1)}%</strong><small>{agents.length} 家公司平均值</small></article><article><span>市场份额合计</span><strong>{totalShare.toFixed(1)}%</strong><small>必须等于 100%</small></article></div><div className="report-grid clean-report"><section className="card report-findings"><span>本次运行说明了什么</span><h3>当前实验结果</h3><article><i>01</i><div><strong>市场竞争已经完成</strong><p>价格和投入共同影响份额与利润，全部公司份额总和保持 100%。</p></div><b>已观察到</b></article><article><i>02</i><div><strong>份额不等于利润</strong><p>份额领先者与利润领先者可能不是同一家公司，需要同时解释竞争和经营结果。</p></div><b>描述性结果</b></article><article className="warn"><i>03</i><div><strong>{runtimeMode === "demo" || ruleAutoRunUsed ? "不能作为真实模型行为证据" : "单个实验不能形成因果结论"}</strong><p>{runtimeMode === "demo" ? "正式结论必须来自后端市场环境、真实智能体、回合事件和过程重建。" : ruleAutoRunUsed ? "规则续跑只验证市场可结算性；研究真实智能体时必须由 Coordinator 完整推进。" : "至少需要共同随机种子、固定处理条件和配对统计。"}</p></div><b>证据边界</b></article></section><section className="card agent-report"><span>智能体最终结果</span><h3>公司结果</h3>{agents.map((agent) => <article key={agent.companyId}><i style={{ background: agent.color }}>{agent.companyId.slice(-1)}</i><div><strong>{agent.persona}</strong><p>{agent.decision.summary}</p></div><span><b>{formatPercent(agent.share)}</b><small>市场份额</small></span><span><b>{formatMoney(agent.profit, true)}</b><small>利润</small></span></article>)}</section></div></div>;
}
...
export default function Home() {
  ...
  async function autoRunRemaining(humanAction: { price: number; advertising: number; contribution: number }) {
    if (demoCompleted) return;
    if (runtimeMode === "demo") {
