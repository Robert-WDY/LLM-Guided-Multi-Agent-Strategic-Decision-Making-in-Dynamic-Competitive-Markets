...
export default function Home() {
  ...
  async function autoRunRemaining(humanAction: { price: number; advertising: number; contribution: number }) {
    ...
    setBusy(true);
    try {
      const response = await fetch(`${API_URL}/episodes/${episodeId}/auto-run`, { method: "POST", headers: { "Content-Type": "application/json", ...(config.controllerToken ? { "X-Controller-Token": config.controllerToken } : {}) }, body: "{}" });
      ...
      setNotice(payload.state.terminal ? "规则代理已跑完剩余回合，实验完成。" : "规则代理已推进剩余回合。");
