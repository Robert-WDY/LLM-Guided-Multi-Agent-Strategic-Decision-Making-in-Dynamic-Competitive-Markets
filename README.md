# Game Theory Agent — Reliable Advisor v19 研究交付

2026-09-08，新增Reliable原计划改进层：显式Agent Draft、局部候选、滞后条件响应、H3/H5验证与弃权，交互H1模式提供目标权重/效用分解，自动保存恢复。市场公式与旧版存档保持冻结。

已完成720源状态、2160次建议、1080局合作制度实验、120局资源规模对照、45局参数敏感性与Stackelberg真值检查。条件建议三轮门槛通过25/36组，三轮负收益24/720、五轮负收益28/720、弃权361/720。交互9次串行实测0.304–0.910秒，完整后端481项、附加16项收费保护/离线请求、前端18项及构建/类型/lint通过。

真实模型阶段状态为blocked_by_preregistered_gate，本轮新增收费调用0次。累计授权30元、保守预留9.993565元；真实模型有效性、现实参数校准和所有市场稳定改善仍未证明。新版本为研究交付，经济门槛不通过不会被工程通过替代。

完整方法、反例与验收见 [v19实验报告](docs/reliable-advisor-results.md)。本机入口 http://localhost:3210/ → 博弈策略实验室 → Reliable：改进原计划。

以下为此前版本的历史记录，预算与进度以各阶段当时状态为准。
